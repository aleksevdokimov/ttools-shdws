...
class BaseDAO(Generic[T]):
    ...
    async def find_paginated(
        ...
        """
        Пагинированный поиск записей.
        
        Args:
            page: Номер страницы (начиная с 1)
            per_page: Количество записей на странице
            filters: Словарь фильтров для WHERE clause
            order_by: Поле для сортировки (по умолчанию id)
            order_desc: Сортировка по убыванию (по умолчанию True)
        ...
        logger.info(
            f"Пагинированный поиск {self.model.__name__}: "
            f"page={page}, per_page={per_page}, filters={filters}, order_by={order_by}"
        )
        try:
            ...
            logger.error(f"Ошибка при пагинированном поиске: {e}")
            raise

    async def count_with_filters(self, filters: dict | None = None):
        """
        ...
        Returns:
            Количество записей
        """
        logger.info(f"Подсчёт количества {self.model.__name__} с фильтрами: {filters}")
        try:
            ...
            result = await self._session.execute(query)
            count = result.scalar()
            logger.info(f"Всего найдено {count} записей")
            return count
        except SQLAlchemyError as e:
